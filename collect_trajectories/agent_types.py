...
# import safe_rl.pg.trust_region as 
...
def flat_concat(xs):
    """Concatenate flattened tensor"""
    return tf.concat([tf.reshape(x, (-1,)) for x in xs], axis=0)


def flat_grad(f, params):
    """Applied flat_concat to gradients"""
...
class Agent:
    ...
    def ensure_satisfiable_penalty_use(self):
        "Penalties on either reward or objective, but not both"
...
class PPOAgent(Agent):
    # implements PPO Clip, which implements clipped advantage
